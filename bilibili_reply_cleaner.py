...
def _ts_to_str(ts):
    """Unix 时间戳(秒) -> 'YYYY-MM-DD HH:MM' 可读字符串；无效时返回 ''"""
    try:
...
def login_by_qrcode(log=print, qr_callback=None):
    ...
    while True:
        ...
        if code == 0:
            ...
            # cookie 优先由 sess.get 自动从响应头 Set-Cookie 获取，
            # 仅当 URL 参数也有值时才覆盖，避免用空值覆盖已有 cookie
            for name in ("SESSDATA", "bili_jct", "DedeUserID"):
                val = params.get(name, "")
                if val:
                    sess.cookies.set(name, val, domain=".bilibili.com")

            # UID 获取优先级：URL 参数 > session cookies > nav 接口
            uid = params.get("DedeUserID", "")
            if not uid:
                try:
                    uid = sess.cookies.get("DedeUserID", "") or ""
                except Exception:
                    uid = ""
            if not uid:
                try:
                    nav = sess.get(NAV_API, timeout=10).json()
                    if nav.get("code") == 0 and nav.get("data", {}).get("mid"):
                        uid = str(nav["data"]["mid"])
                        log(f"从 nav 接口获取到 UID：{uid}")
                    else:
                        log(f"nav 接口未返回 UID：code={nav.get('code')} message={nav.get('message','')}")
                except Exception as e:
                    log(f"通过 nav 接口获取 UID 失败：{e}")

            # bili_jct 同样可能不在 URL 参数里，从 session cookies 兜底
            bili_jct = params.get("bili_jct", "")
            if not bili_jct:
                try:
                    bili_jct = sess.cookies.get("bili_jct", "") or ""
                except Exception:
                    bili_jct = ""
            ...
            if qr_callback:
                try:
                    ...
                    pass
            log(f"扫码登录成功！UID={uid}\n")
            return sess, uid, bili_jct
        elif code == 86090:
            log("已扫描，请在手机上点击确认 ...")
        elif code == 86101:
            pass  # 等待扫码，不刷屏
...
def fetch_my_replies(session, uid, log=print):
    if not uid or not str(uid).strip().isdigit():
        log(f"UID 无效（uid={uid!r}），无法从 aicu.cc 拉取评论。请重新扫码登录。")
        return []
    ...
    while True:
        ...
        for item in replies:
            try:
                ...
                ctime = int(item["time"]) if item.get("time") else None
                results.append({
                    "rpid": int(item["rpid"]),
    ...
    for label, base_url, time_field in sources:
        ...
        while True:
            ...
            for it in items:
                ...
                # 评论时间：like 的 item.ctime 是评论发表时间；
                # reply 无 ctime，用外层 reply_time（被回复时间）作为时间参考
                ctime = None
                if item.get("ctime"):
                    try:
                        ...
                        pass
                if ctime is None and it.get(time_field):
                    try:
                        ctime = int(it[time_field])
                    except (ValueError, TypeError):
                        pass
...
def preview(replies, log=print):
    ...
    for i, r in enumerate(replies[:20], 1):
        ...
        if len(msg) > 60:
            msg = msg[:60] + "..."
        t = r.get("ctime_str") or "未知日期"
        log(f"  [{i:2d}] {t}  rpid={r['rpid']} oid={r['oid']}")
        log(f"       {msg}")
    if len(replies) > 20:
...
# ---------------------- 日期选择窗口 ----------------------
def pick_date_dialog(replies, log=print, parent=None):
    """
    弹出日期选择窗口，让用户选择一个截止日期。
    parent: 父窗口（GUI 模式传入主窗口，用 Toplevel 避免多 mainloop 冲突）
    返回值：
      int 时间戳 -> 只删除 ctime < 该时间戳 的评论
      -1         -> 删除全部（不按日期过滤）
      None       -> 用户取消，不删除
    """
    try:
        ...
        log("系统没有 tkinter，无法弹出日期选择窗口，将删除全部。")
        return -1

    times = [r.get("ctime") for r in replies if r.get("ctime")]
    if not times:
        log("所有评论都没有日期信息，跳过日期选择，将删除全部。")
        return -1
    ...
    # GUI 模式（有父窗口）用 Toplevel；CLI 模式用 Tk
    ...
    tk.Label(win, text=(
        f"共 {len(replies)} 条评论\n"
        f"日期范围：{min_dt.strftime('%Y-%m-%d')} ~ {max_dt.strftime('%Y-%m-%d')}\n"
        f"将删除所选日期【之前】的评论"
    ...
    years = list(range(min_dt.year, today.year + 1))
    months = list(range(1, 13))
    days = list(range(1, 32))

    sv_year = tk.StringVar(value=str(max_dt.year))
    sv_month = tk.StringVar(value=f"{max_dt.month:02d}")
    sv_day = tk.StringVar(value=f"{max_dt.day:02d}")

    tk.Label(frame, text="年").grid(row=0, column=0)
    tk.Label(frame, text="月").grid(row=0, column=1)
    tk.Label(frame, text="日").grid(row=0, column=2)

    ttk.Combobox(frame, textvariable=sv_year, values=[str(y) for y in years], width=8, state="readonly").grid(row=1, column=0, padx=5)
    ttk.Combobox(frame, textvariable=sv_month, values=[f"{m:02d}" for m in months], width=5, state="readonly").grid(row=1, column=1, padx=5)
    ttk.Combobox(frame, textvariable=sv_day, values=[f"{d:02d}" for d in days], width=5, state="readonly").grid(row=1, column=2, padx=5)

    count_label = tk.Label(win, text="", fg="blue")
    count_label.pack(pady=10)

    def update_count(*_):
        try:
            y, m, d = int(sv_year.get()), int(sv_month.get()), int(sv_day.get())
            cutoff = int(datetime(y, m, d).timestamp())
            n = sum(1 for r in replies if r.get("ctime") and r["ctime"] < cutoff)
            count_label.config(text=f"将删除 {n} 条（共 {len(replies)} 条）")
        except (ValueError, TypeError):
            count_label.config(text="日期无效")

    for sv in (sv_year, sv_month, sv_day):
        sv.trace_add("write", update_count)
    update_count()
    ...
    def on_ok():
        try:
            y, m, d = int(sv_year.get()), int(sv_month.get()), int(sv_day.get())
            result["ts"] = int(datetime(y, m, d).timestamp())
        except ValueError:
            result["ts"] = None
        win.destroy()
    ...
    tk.Button(btn_frame, text="删除此日期之前", command=on_ok, width=16).grid(row=0, column=0, padx=5)
    tk.Button(btn_frame, text="全部删除", command=on_all, width=10).grid(row=0, column=1, padx=5)
    tk.Button(btn_frame, text="取消", command=on_cancel, width=8).grid(row=0, column=2, padx=5)
...
# ---------------------- 主流程 ----------------------
def run_pipeline(log=print, confirm_func=None, progress_func=None, qr_callback=None, auto_confirm=False, parent=None, session=None, uid=None, bili_jct=None):
    """
    运行完整的删除流程。可通过回调参数适配 CLI / GUI。
      ...
      auto_confirm:  跳过确认
      parent:        GUI 主窗口（用于日期选择窗口的父窗口）
      session/uid/bili_jct: 复用已有登录态，不传则重新扫码登录
    """
    log("== B站历史回复批量清理工具（可靠验证版） ==\n")

    if not session or not bili_jct:
        session, uid, bili_jct = login_by_qrcode(log=log, qr_callback=qr_callback)
    elif not uid:
        # session 有效但 uid 缺失，先用 nav 接口补取，取不到再重新扫码
        log("已有登录态但 UID 缺失，尝试通过 nav 接口获取 ...")
        try:
            nav = session.get(NAV_API, timeout=10).json()
            if nav.get("code") == 0 and nav.get("data", {}).get("mid"):
                uid = str(nav["data"]["mid"])
                log(f"获取到 UID={uid}\n")
            else:
                log("nav 接口未返回 UID，需要重新扫码登录。\n")
                session, uid, bili_jct = login_by_qrcode(log=log, qr_callback=qr_callback)
        except Exception as e:
            log(f"nav 接口请求失败：{e}，需要重新扫码登录。\n")
            session, uid, bili_jct = login_by_qrcode(log=log, qr_callback=qr_callback)
    else:
        log(f"使用已有登录状态，跳过扫码。UID={uid}\n")
    ...
    for rpid, r in msgfeed_replies.items():
        if rpid in merged:
            ...
            # 时间：aicu 的 time 是评论发表时间，优先；aicu 没有时才用 msgfeed 的
            if not merged[rpid].get("ctime") and r.get("ctime"):
                merged[rpid]["ctime"] = r["ctime"]
                merged[rpid]["ctime_str"] = r.get("ctime_str", _ts_to_str(r["ctime"]))
        else:
            merged[rpid] = r
            msgfeed_new += 1
    ...
    if not replies:
        log("没有拉到任何评论，结束。")
        return {"session": session, "uid": uid, "bili_jct": bili_jct}

    replies = apply_filter(replies, log=log)

    # 按日期过滤：弹出窗口让用户选择截止日期
    cutoff = pick_date_dialog(replies, log=log, parent=parent)
    if cutoff is None:
        log("已取消。")
        return {"session": session, "uid": uid, "bili_jct": bili_jct}
    if cutoff != -1:
        before = len(replies)
        replies = [r for r in replies if not r.get("ctime") or r["ctime"] < cutoff]
        log(f"日期过滤：只删除 {_ts_to_str(cutoff)} 之前的评论，{before} -> {len(replies)} 条\n")
