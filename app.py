...
class App:
    ...
    def on_qr_callback(self, pil_img):
        """在独立窗口显示二维码；收到 None 表示登录成功，关闭窗口"""
        if pil_img is None:
    ...
    def _worker(self):
        try:
            ...
            if isinstance(result, dict):
                ...
                self.bili_jct = result.get("bili_jct")
    ...
    def _on_done(self):
        self.is_running = False
        # 已登录则按钮显示"再次运行"，否则"开始"
        btn_text = "再次运行" if self.session else "开始"
        self.btn_start.config(state=tk.NORMAL, text=btn_text)
        self.log("\n— 运行结束 —")
